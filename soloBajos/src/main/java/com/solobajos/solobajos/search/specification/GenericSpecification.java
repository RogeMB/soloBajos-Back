...
public class GenericSpecification<T> implements Specification<T> {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query,
        ...
        if (operator.equalsIgnoreCase(">")) {
            ...
            if (isString(type)) {
                // .toCharArray[0]=='*';  también nos valdría en lugar del statsWith
                if(searchCriteria.getValue().toString().startsWith("*",0)){
                    String startWith = searchCriteria.getValue().toString().substring(1);
                    return criteriaBuilder.like(
                            root.get(searchCriteria.getKey()), startWith + "%");
                } else {
                    return criteriaBuilder.like(
